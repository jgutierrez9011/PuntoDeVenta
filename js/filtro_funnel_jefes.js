...
/*FILTRO EN PESTAÑA DE BASE DE CLIENTES*/
$('#txttienda_base_clientes_4').change(function()
{
  ...
  {
    $( '#txtempleado_filtro_base_clientes_1' ).html( respuesta );

     $('#txtempleado_filtro_base_clientes_1').selectpicker('refresh');

  });
});

/*FILTRO EN PESTAÑA DE MIS DE CLIENTES*/
$('#txtempleado_filtro_mis_clientes_4').change(function()
{
  var el_tienda = $(this).val();
  ...
  $.post('fnsvaclient.php', { var_tienda: el_tienda} ).done( function( respuesta )
  {
    $( '#txtempleado_filtro_mis_clientes_1' ).html( respuesta );

     $('#txtempleado_filtro_mis_clientes_1').selectpicker('refresh');

  });
});

/*FILTRO EN PESTAÑA DE EMBUDO DE VENTAS*/
$('#txttienda_filtro_funnel_4').change(function()
{
  var el_tienda = $(this).val();
  ...
  $.post('fnsvaclient.php', { var_tienda: el_tienda} ).done( function( respuesta )
  {
    $( '#txtempleado_filtro_funnel_1' ).html( respuesta );

     $('#txtempleado_filtro_funnel_1').selectpicker('refresh');

  });
});

/*MUESTRA LOS CLIENTES DE BASE DE PROSPECTOS SEGUN LOS FILTROS PARA CUANDO EL USUARIO SEA JEFE*/
$('#btnfiltrar_base_clientes_1').click(function(){

           //event.preventDefault();
      var zona_basepros_gerencia_ = '',
          tienda_basepros_gerencia_ = document.querySelector('#txttienda_base_clientes_4').value,
          ejecutivo_basepros_gerencia_  =  document.querySelector('#txtempleado_filtro_base_clientes_1').value;
...
$('#btnfiltrar_mis_clientes_1').click(function(){

                             //event.preventDefault();
var zona_mispros_gerencia_ = '',
    tienda_mispros_gerencia_ = document.querySelector('#txtempleado_filtro_mis_clientes_4').value,
    ejecutivo_mispros_gerencia_  =  document.querySelector('#txtempleado_filtro_mis_clientes_1').value;
...
Viewmoredata_jefes = function () {

  var zona_funnel_gerencia_ = '',
      tienda_funnel_gerencia_  = document.querySelector('#txttienda_filtro_funnel_4').value,
      carnet_funnel_gerencia_ = document.querySelector('#txtempleado_filtro_funnel_1').value,
      inicia_periodo_funnel_gerencia_ = document.querySelector('#periodo_inicia_filtro_funnel_1').value,
      finaliza_periodo_funnel_gerencia_  = document.querySelector('#periodo_finaliza_filtro_funnel_1').value;
...
var buttons_jefes = Highcharts.getOptions().exporting.buttons.contextButton.menuItems;

                      buttons_jefes.push({
                       text: "View more data",
                       onclick: Viewmoredata_jefes
                       });


/*Crea el embudo a partir de filtros JEFES*/
function fn_draw_funnel_sales_filter_jefes(valores)
{
...
menuItems: buttons_jefes
 ...
 /*MUESTRA EL EMBUDO DE VENTAS CUANDO EL USUARIO ES EL JEFE O SUPERVISOR*/
  $('#btnfiltrar_filtro_funnel_1').click(function(event){
   ...
   $.ajax({
           ...
           {
               $('#content_funnel').html('');
               fn_draw_funnel_sales_filter_jefes(data);
           }
         })
